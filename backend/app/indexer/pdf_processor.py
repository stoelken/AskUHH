...
MIN_AREA         = 10_000   # raster/xobject: ~100×100 pt ≈ 3.5×3.5 cm
MIN_AREA_DRAWING = 25_000   # cluster_drawings: stricter, more noise there
MIN_DIM          = 50       # both width and height must exceed this (pt)
MIN_ASPECT       = 0.15     # filter out thin horizontal strips
MAX_ASPECT       = 6.5      # filter out thin vertical strips
...
def _overlap_ratio(a: fitz.Rect, b: fitz.Rect) -> float:
    """Intersection area / area of the smaller rect."""
    inter = a & b
    if inter.is_empty:
        return 0.0
...
def extract_images_from_pdf(
    ...
    results = []
    seen_hashes: set[str] = set()  # pixel-content dedup across all pages
    ...
    for page_num in range(len(doc)):
        ...
        # 3) vector drawing clusters — stricter threshold
